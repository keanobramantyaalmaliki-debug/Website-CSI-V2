...
> ⚠️ **Test suite: 322 test / 53 berkas, hijau** (23 Agu; naik dari 318/52 — plus 4 test `clampFreeBall` di berkas baru `freeBall.test.ts` §4ay; `Process.test.tsx` ditulis ulang untuk layout tali §4ax dengan jumlah test tetap). Satu test *pernah* merah pada satu putaran penuh — `TheCrewMobileCarousel.test.tsx` ("auto-advances 30s after going idle") — lalu hijau saat dijalankan sendiri **dan** pada putaran penuh berikutnya. **Flake fake-timer di bawah beban, bukan regresi**; kalau ia muncul lagi, curigai `advanceTimersByTimeAsync` di suite yang berbagi sesi timer, bukan komponennya.
...
- **Vitest 4** — `bun run test`. **322 test di 53 berkas**, semuanya hijau per 23 Agu (sempat turun ke 318/52 saat `IndustriesMobile` pensiun §4aw, lalu naik lagi berkat `freeBall.test.ts` §4ay). Empat di antaranya invariant lintas-wilayah (`INVARIANTS.md` §1, §3, §6, §7). Norma repo: **buktikan test-nya MERAH di kondisi rusak dulu** sebelum dipakai memverifikasi perbaikan
