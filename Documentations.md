...
- **Minigame billiard dibangun** (§6d) — fisika **cannon-es** (bukan Rapier lagi, lihat §6d) + kamera top-down + bar tenaga. ✅ **Kini FINAL** — direview di browser 12 Agu, clear tanpa revisi; dibuka lagi 23 Agu untuk **free ball** (§4ay).
...
- **Industries: galeri → tangga spiral 3D** (§4au) — porting pmndrs `raycast-cycling` di strip putih full-bleed 85svh: 13 plank kaca buram (komposisi demo verbatim), hover = tint + HUD, klik plank = mode fokus (terbang kiri + expand foto + panel deskripsi, sisa tangga & bayangan fade). Heading pindah KE DALAM strip; eyebrow + counter dihapus. **Wheel-cycling sempat dibangun lalu DICABUT di hari yang sama** (klik lebih jelas; wheel = scroll halaman murni) — sisa fork `<CycleRaycast/>` tinggal saringan `visible`. Canvas KETIGA di halaman, demand. 🐛 ref material lupa dipasang = rantai invalidate mati diam-diam; 🔥 raycaster three tidak cek `visible`.
- **Bersih-bersih kode mati & aset sisa** (§4av) — eksekusi backlog audit 23 Agu: 7 komponen tak terpakai + keyframes marquee + `export-test/` lama (termasuk GLB backup 8,4 MB) + aset nganggur; −2.279 baris, nol perubahan perilaku.
- **Industries dibawa ke mobile** (§4aw) — urutan spiral dibalik (**01 di puncak**), gerbang perangkat dihapus & `IndustriesMobile` pensiun: satu stack 3D untuk semua. Kamera mundur responsif di kanvas sempit (rasio penuh, varian sqrt gagal), fokus layar sempit = kartu atas + panel lembar bawah (teks dikecilkan, back disembunyikan), dan **navigasi sektor `‹ 01 Nama ›`** di pointer kasar (ide Keano): arrow wrap-around **berarah spasial** (kiri = nomor naik), nama = tombol buka, plank ter-tint. 🐛 kolom nama wajib fixed-width — kalau melar, kedua arrow ikut bergeser & tap beruntun meleset.
- **Process: daftar + sticky panel → "tali" scroll-drawn** (§4ax) — satu goresan SVG putih 12 px menembus PUSAT 6 kartu vertikal berselang-seling; kartu "menggembung" dari garis tali (analogi ular menelan kotak), ekornya menyatu ke strip Industries yang sewarna. `StickyScroll` + `useScrollStepper` dihapus dari repo. 🔑 path dibangun dari ukur `offsetTop` (BUKAN viewBox stretch / `getBoundingClientRect`) + remap scroll→busur piecewise (linear bikin ujung tali tertinggal).
...
> ✅ **`submitInquiry()` sudah bukan stub** sejak 26 Agu — lihat **§4bd**. Taruhan desainnya terbayar persis seperti niatnya: karena seluruh jalan keluar form dikurung dalam satu fungsi, memasang Web3Forms **tidak menyentuh satu baris pun** di `InquiryLaptop.tsx` maupun rig di section ini. Yang berubah hanya isi fungsi itu dan `ContactForm.tsx` (peringatan isian). Jeda 900 ms tiruan itu pun sudah selesai tugasnya: keadaan "sending" memang sudah teruji sebelum backend ada, jadi saat latensi sungguhan masuk, tak ada yang perlu dibetulkan.
...
- **Revisi malam (batch permintaan Keano, final)**: (1) **wheel-cycling DICABUT TOTAL** — dengan click-to-open, menggilir lapisan via wheel redundan; override filter perotasi intersection, penyandera wheel, dan HUD titik lapisan dibuang, wheel di atas strip = scroll halaman murni; sisa fork tinggal `VisibleFilter` (saringan `visible` tetap wajib untuk mode fokus). (2) Section = **strip-nya saja**: latar `bg-white/[0.02]` + `border-y` + blok header gelap dihapus; heading + subtext jadi overlay kiri-atas DI DALAM strip (ikut aria-hidden; `h2` sr-only di `Industries.tsx` untuk AT + test), ikut pudar saat fokus; eyebrow "Industries" + counter "13 SECTORS · 3 core" dicabut (ada test negatifnya). (3) Strip ditinggikan **70svh → 85svh** (min 520px) — fov tetap, cluster ikut membesar. (4) Label idle "13 sectors, one stack" dihapus, tapi **panduan interaksi dipertahankan**: "Hover the stack" (idle) → "Click to open" (hover); teks plank di HUD dibuat `justify-end` supaya sejajar dasar dengan panduan (min-h kiri tadinya membuat teksnya mengambang).

---

## 4av. Bersih-bersih Kode Mati & Aset Sisa ✅ (23 Agu)

Eksekusi item "kode mati" dari backlog audit performa 23 Agu — **−2.279 baris / 18 berkas tanpa satu pun perubahan perilaku** (typecheck + suite penuh tetap hijau). Yang pergi:

- **7 komponen tak lagi diimpor siapa pun**: `Manifesto.tsx` + `ManifestoField.tsx` (section-nya sudah dicabut dari Lounge), `DeploymentsField.tsx` + `DeploymentsMatterField.tsx` (menganggur sejak rombak Deployments; yang kedua bekas eksperimen matter-js), `backgrounds/NetworkField.tsx`, `PinnedServiceStack.tsx` (Services kini sabuk ticker §4aq), `ui/number-ticker.tsx` (panel stat dibuang §4ar). Referensi tersisa hanya komentar sejarah di `Scene.tsx`/`RoomContent.tsx`/`Office.tsx` — sengaja dibiarkan, mereka menjelaskan kenapa kode sekitarnya berbentuk begitu.
- **`@keyframes marquee-scroll`** di `index.css` — pemakainya (marquee Industries lama) sudah lama pergi.
- **Artefak & aset nganggur**: `export-test/` seutuhnya (viewer HTML era MVP1 + `office-mvp1-baked.glb` 8,4 MB — perannya sudah lama digantikan app utama), `apple-logo.png` di root, `public/home/P1330392_noir.webp` (varian noir tak terpakai; ingat §4ai — semua isi `public/` disalin Vite jadi URL publik), `diskusi/4-Agustus copy.md` (duplikat).

---

## 4aw. Industries Dibawa ke Mobile: Satu Stack Semua Perangkat + Navigasi Sektor ✅ (23 Agu)

Sesi mobile §4au. Dimulai dari perbaikan urutan, berakhir dengan carousel lama pensiun dan tumpukan 3D yang sama tampil di semua perangkat.

- **Urutan spiral DIBALIK** — pose memakai indeks terbalik `k = n−1−i`: sektor **01 di anak tangga TERATAS**, 13 terbawah (geometri spiral identik, hanya penempatan sektor bertukar ujung; sebelumnya `i` langsung membuat 01 di dasar).
- **Gerbang `isDesktop && !coarse` DIHAPUS** dari `Industries.tsx` — alasan lamanya (wheel-cycling butuh hover + wheel) sudah gugur sejak wheel dicabut (§4au); interaksi tinggal tap/klik yang jalan di mana saja. **`IndustriesMobile` + test-nya dihapus dari repo** (karenanya hitungan suite turun); heading fallback ikut pergi, `h2` sr-only + `<ul>` sr-only jadi satu-satunya jalur AT.
- **Kamera responsif (`CameraRig`)** — posisi demo dipertahankan sebagai *arah*, jaraknya dikali `k = min(WIDE_ASPECT/aspect, 2.2)` saat aspek kanvas < 1,4, plus `lookAt(0,0,0)` eksplisit (mode fokus men-slerp plank ke quaternion kamera — orientasi tidak boleh mengandalkan default R3F). ⚠️ Varian `sqrt(ratio)` DICOBA & GAGAL: ekor spiral masih terpotong tepi kanan di 390×844; rasio penuh yang lolos screenshot.
- **Pose fokus dua varian** — lebar: kartu kiri (`NDC −0.45/−0.04`, dist 8,8) + panel kanan `w-1/2`; sempit (< 768 px, sengaja = breakpoint `md:` panel DOM supaya canvas & CSS sepakat): kartu **atas-tengah** (`NDC 0/+0.3`, dist 11,5 — lebih jauh supaya kartu mengecil) + panel jadi **lembar bawah**. Revisi Keano menyusul: teks panel di bawah `md` dikecilkan + margin dirapatkan (judul `text-lg`, desc `text-sm` — sempat mepet foto) dan tombol "Back to the stack" `max-md:hidden` (ruangnya jatuh ke deskripsi; menutup cukup tap kartu / area kosong via `onPointerMissed`).
- **Pointer kasar: hover DILEWATI** — `pointerover` sentuh ikut tap dan *menempel* sampai tap berikutnya, jadi HUD hover menyesatkan; tap langsung membuka fokus.
- **Navigasi sektor `‹ 01 Nama Sektor ›`** (ide Keano) — plank tipis + bertumpuk + miring perspektif = target tap kecil yang gampang meleset ke tetangganya. Di pointer kasar baris bawah HUD diganti navigasi tengah-bawah: arrow menggilir wrap-around, **plank terpilih di-tint aksen di canvas** (prop `navActive`, ikut deps effect invalidate — frameloop demand harus dibangunkan), dan **namanya sendiri tombol pembuka fokus** (tanpa tombol enter terpisah, disepakati saat diskusi). Tap langsung di plank tetap hidup dan **menyinkronkan** navIndex (`selectAndSync`) — sesudah fokus ditutup, navigasi menunjuk sektor yang barusan dibuka. Tombol `touch-manipulation` + `tabIndex −1` (seisi strip aria-hidden).
- 🐛 **Gotcha arrow bergeser — kolom nama WAJIB fixed-width** (`w-[15.5rem]`): barisnya `justify-center`, jadi kolom tengah yang melar mengikuti panjang nama menggeser kedua arrow ±48 px tiap sektor berganti → tap beruntun meleset. Ketahuan di probe: tap kedua bahkan tidak menghasilkan `touchstart` di tombolnya (instrumentasi counter click/touch per tap yang membongkarnya — dua hipotesis pertama, double-tap-zoom & koordinat basi karena scroll, dua-duanya salah).
- **Arah arrow SPASIAL, bukan urutan nomor** (revisi Keano) — di komposisi ini 01 duduk di kanan-atas dan nomor naik ke arah kiri, jadi arrow **kiri = nomor naik**, arrow **kanan = nomor turun** (dua-duanya wrap). Kalau komposisi spiral berubah, pemetaan ini wajib dicek ulang (ada komentarnya di kode).
- **Verifikasi**: probe baru `scripts/shoot-industries-mobile.mjs` — viewport 390×844 + `Emulation.setDeviceMetricsOverride` **dan** `setTouchEmulationEnabled` (tanpa itu `(pointer: coarse)` tidak match, pola `drive.mjs` §7); mengetes gilir arrow di koordinat yang sama (regresi bug bergeser), buka-dari-nama, tutup-tap-kosong, dan sinkron tap-plank. Probe desktop `shoot-industries-stack.mjs` dijalankan ulang — framing, hover, wheel-scroll, fokus semuanya identik (aspek lebar → `k=1`).
- **Test jsdom navigasi** (overlay-nya DOM murni, canvas tidak perlu): ⚠️ role-query Testing Library **tidak menemukan tombol di dalam subtree `aria-hidden`** — ambil lewat `querySelector`; ⚠️ panel exit AnimatePresence **masih di DOM** sampai animasinya selesai — jangan assert absennya deskripsi sesudah back, perjalanan pulang diuji di probe browser.

---

## 4ax. Process: Daftar + Sticky Panel Diganti "Tali" Scroll-Drawn ✅ (23 Agu malam)

Layout lama Process (daftar teks `FadeUpList` di kiri + panel sticky `StickyScroll` di kanan yang mengganti glyph per langkah, disinkronkan `useScrollStepper`, plus `MobileGlyph` sebagai fallback sentuh) DIGANTI TOTAL dengan satu mekanisme untuk semua perangkat: **"tali" SVG putih yang menjalar mengikuti scroll** (adaptasi Skiper19/comgio). `sticky-scroll-reveal.tsx` + `useScrollStepper.ts` **dihapus dari repo** — pensiun bersama layout lama.

**Bentuknya**: satu goresan `#fafafa` (zinc-50) 12 px masuk dari sisi kanan, menukik melewati tepi kanan teks heading, lalu **MENEMBUS PUSAT keenam slot kartu** yang kini bertumpuk vertikal berselang-seling kiri–kanan — masuk tepat di tengah tepi atas kartu, keluar tepat di tengah tepi bawahnya, jadi tali dan kartu selalu segaris. Begitu ujung tali selesai melewati sebuah slot, kartunya **menggembung dari garis tali** — analogi Keano: tali = ular, kartu = kotak yang ditelan; perutnya membesar simetris kiri-kanan (clip-path membuka dari bilah selebar goresan di pusat ke lebar penuh, 0,8 dtk; isi teks gelap menyusul delay 0,45). Kartu kini TERANG (`#fafafa`, sewarna tali — terbaca satu benda) dengan teks zinc-900; glyph `ProcessGlyphs` ikut di-tuning kontras (opacity & tebal goresan naik) karena pindah dari latar gelap ke terang. **Dua varian DITOLAK Keano**: tali menyusuri tepi atas + kartu "dituang" ke bawah (terbaca menggantung kartu), dan tali membungkus keliling kartu. **Ekor tali** turun sendirian melewati landasan kosong 45svh lalu menyentuh strip `IndustriesStack` yang juga zinc-50 — dua section terbaca menyatu tanpa gradasi.

- **Path DIBANGUN dari ukuran, bukan viewBox statis yang di-stretch** — `preserveAspectRatio="none"` membuat tebal goresan berubah-ubah arah dan merusak normalisasi dash `pathLength`. Path dihitung ulang dari posisi kartu hasil ukur **`offsetTop`/`offsetLeft`** (sengaja BUKAN `getBoundingClientRect` — transform animasi kartu ikut terbaca dan meracuni pengukuran); div terukur dibiarkan polos, semua animasi hidup di `motion.div` anaknya.
- **Remap scroll→panjang busur PIECEWISE, bukan linear** — panjang busur tidak sebanding posisi vertikal (sapuan masuk + kelokan S "makan" busur), jadi `pathLength` yang dipetakan linear dari scroll membuat ujung tali makin tertinggal di atas viewport. Solusi: pasangan `stops[i]` (fraksi scroll saat slot ke-i berada di garis pandang `TRIGGER_VH` 0,75) → `thresholds[i]` (fraksi busur saat ujung tali selesai melintasi slot itu, via sampling panjang kubik) jadi titik patah `useTransform` — ujung tali selesai melewati kartu persis saat penggunanya memandang slot itu. Progress `useScroll` `start 0.85` → `end 0.5` (ujung selesai memutih saat perbatasan Process→Industries masih di tengah layar).
- **Ukur tepi kanan heading via Range di TEXT NODE terdalam** — span pembungkus `LineMask` ber-display block, `selectNodeContents(h2)` mengembalikan kotak selebar `max-w-xl`, bukan lebar teks; jsdom tidak punya `Range.getBoundingClientRect` → fallback 0,5×lebar untuk lingkungan test.
- **Clip-path pakai inset PERSEN** (~(lebar − STROKE)/2) karena motion tidak bisa menginterpolasi `calc()`; selisih ±1 px dari 12 px tertutup goresan tali di belakangnya. Reduced motion: `pathLength` langsung 1 + clip terbuka.
- **Re-measure**: ResizeObserver di wrapper + listener `resize` window — `stops` bergantung `innerHeight` dan tinggi wrapper pakai svh, jadi RO tidak selalu terpicu saat vh berubah (URL bar mobile).
- **Mobile**: kartu dirampingkan `min(20rem, 68vw)` supaya pusat kartu ganjil/genap benar-benar berseberangan dan kelokan tali terbaca zig-zag.
- **Test** `Process.test.tsx` ditulis ulang (tiap langkah tampil SEKALI, 6 svg glyph; SVG tali absen di jsdom karena wrapper berukuran nol — dan butuh stub `ResizeObserver`). Verifikasi visual: probe baru `scripts/shoot-process-rope.mjs` (beberapa titik progres + perbatasan Process→Industries).

---

## 4ay. Billiard Dibuka Lagi: Free Ball Zona Kitchen + Power Bar Dibalik + HUD Kontras ✅ (23 Agu)

§6d ditutup FINAL 12 Agu, dibuka lagi untuk satu fitur gameplay: **free ball ("ball in hand")** — bola putih boleh digeser di zona kitchen sebelum break dan setiap kali bola putih masuk lubang. State `ballInHand` di store: nyala saat masuk/reset dan saat bola putih di-respot ke head spot, **padam begitu tembakan dilepas**.

- **Zona kitchen = 35% panjang meja dari ujung head** (`KITCHEN_Z0` di `table.ts`); head spot ada DI DALAM zona, jadi respot bawaan tidak pernah melanggar batasnya sendiri. `clampFreeBall` murni geometri: semua sisi dikurangi `BALL_R` — di bantalan supaya tidak spawn menembus rail, di garis batas supaya SELURUH bola di dalam zona (pusat tepat di garis = separuh bola nongol keluar, batasnya terlihat bohong). **4 test murni** di `freeBall.test.ts`; interaksi drag-nya diverifikasi mata via `drive.mjs`.
- **Drag ditangkap bidang raycast tak terlihat di atas felt** (bukan listener window seperti aim di HUD): `e.point` langsung memberi posisi dunia di bidang meja tanpa unproject manual; bidangnya sedikit lebih luas dari felt supaya tarikan melewati bantalan tetap terbaca lalu dijepit clamp, plus `setPointerCapture` supaya tarikan cepat keluar bidang tidak nyangkut. Genggaman hanya dimulai dalam radius `CUE_GRAB_R = 2,2×BALL_R` dari bola putih — sisanya lolos ke geser-membidik HUD.
- ⚠️ **Kontrak dua radius**: zona mati aim di HUD (48 px layar sekitar `cueScreen`) HARUS lebih besar dari `CUE_GRAB_R` (≈ 42 px pada kamera billiard standar). Kalau terbalik, ada cincin tempat KEDUANYA aktif — bola ikut pointer sambil stik berputar.
- **Posisi menimpa bola lain DIABAIKAN** (bola berhenti di posisi sah terakhir), bukan didorong solver — solver cannon-es yang menyelesaikan dua bola saling tembus akan meledakkannya keluar.
- **Penanda zona = garis head string saja** (putih 0.4) — sorotan bidang zona pernah dibangun dan **DICABUT atas permintaan Keano**: mewarnai 35% meja terlalu ramai. Hint HUD ikut kondisi: "Free ball — drag it inside the zone · drag elsewhere to aim".
- **Power bar DIBALIK: tarik ke BAWAH untuk mengisi** — seperti menarik stik ke belakang sebelum menembak; isian tumbuh dari atas. Gradasi hijau→orange→merah dipatok setinggi bar penuh dan dijangkarkan ke atas lalu terpotong tinggi isian, jadi merah hanya tampak mendekati 100%. ⚠️ **TANPA transition** — garis pegangan mengikuti pointer mentah, transisi 75 ms membuat warna terseret telat di belakang garis.
- **Pass kontras HUD** (teks /40→/80, bar & tombol diberi `bg-black/50` + border lebih terang, hover invert) — HUD lama nyaris tak terbaca di atas felt gelap.
- **`drive.mjs` dapat 3 langkah lagi**: `down` / `holdmove` / `up` — menahan power bar sambil dipotret; `drag` selalu melepas di akhir (= menembak), jadi tidak bisa dipakai untuk memotret keadaan tertahan.

---

## 4az. Susulan Industries Stack: Plank Telat 2–3 Detik + Plank Menyelam saat Transit ✅ (23 Agu)

Dua perbaikan menyusul §4au/§4aw, keduanya dari laporan Keano.

- 🔥 **"Strip putihnya ada tapi plank baru muncul 2–3 detik kemudian"** — gotcha mount R3F di tengah scroll. Canvas Industries SELALU mount saat halaman sedang bergulir (gerbang `useInView`), dan `react-use-measure` bawaan `<Canvas>` melewatkan ResizeObserver **dan** event scroll lewat SATU debounce 50 ms yang sama — tiap event scroll me-reset timernya, jadi selama Lenis masih meluncur ukuran container tidak pernah keluar, R3F **menahan pembuatan root + konteks WebGL**, dan strip tampil kosong sampai scroll benar-benar diam. Fix satu baris: `resize={{ scroll: false, debounce: { scroll: 50, resize: 0 } }}` — listener scroll-nya memang tidak dibutuhkan (rect posisi tidak dipakai; pointer R3F pakai `offsetX/Y`). Diagnosis lewat probe baru `scripts/probe-industries-late-planks.mjs`: load dingin (cache dimatikan via CDP) + wheel beruntun sampai strip masuk viewport + screenshot tiap ~250 ms, disandingkan dengan timeline `PerformanceObserver` longtask dan `MutationObserver` "kapan `<canvas>` muncul" (opsional throttle kbps untuk meniru unduhan GLB publik).
- 🐛 **Plank transit fokus "menyelam"** di balik plank yang dilewatinya sebelum mereka sempat fadeout (revisi Keano). Fix: selama progress transit hidup (`p > 0.02`) `depthTest` material dimatikan + `renderOrder` dinaikkan — plank TERPILIH order 20, di atas plank yang sedang mengalir pulang order 10 (dua-duanya bisa transit berbarengan saat pindah fokus cepat), foto anak selalu +1 di atas plank pembawanya; semuanya dipulihkan begitu plank duduk lagi di spiral.

---

## 4ba. PhysicsHeading & AwardsShowcase Dicabut + Foto Deployments Terang Penuh ✅ (25 Agu)

- **Deployments:** heading physics (matter-js rigid body per kata, jatuh saat kursor masuk) diganti `<h2>` diam + `LineMask` — idiom yang sama dengan h2 pembuka section lain, dan persis cabang prefers-reduced-motion komponen lama, jadi tampilan diamnya tidak berubah. `PhysicsHeading.tsx` + test dihapus. ⚠️ matter-js kini tak dipakai kode mana pun (sisa referensi cuma komentar pelajaran "laptop panas" §4r); dependensinya sengaja belum dilepas dari package.json.
- **Foto kartu Deployments:** puncak kecerahan disamakan jalur mouse & sentuh — `group-hover:opacity-70` → 100, reveal scroll 0.72 → 1 (tepi tetap 0.4). Foto terang penuh saat kartu di tengah layar.
- **Office:** `AwardsShowcase` (daftar award dummy + 3 foto founder `public/people/fami-*.jpg`) dicabut berikut test & asetnya. Section kini ditutup TestimonialSpotlight, yang margin-nya berubah `my-*` → `mt-*` — sebagai anak terakhir section, margin-bawahnya menumpuk dengan padding section + Contact (280px vs 112–128px di ruangan lain). **Aturan: anak terakhir section ber-margin-bawah nol.**
- 🐛 **Fix test laten** `DeploymentsMobileReveal`: stub IntersectionObserver dipasang per-test (`beforeEach`), bukan sekali saat modul dimuat — `vi.unstubAllGlobals()` di `afterEach` menyapu stub level-modul, dan test kedua meledak begitu heading pakai LineMask.

## 4bb. QC Zoom-Out Browser: Jepitan dpr + Kolom Konten `section-shell` ✅ (26 Agu)

Keluhan Keano: browser di-zoom-out → scene 3D frame drop + banyak bagian halaman "tidak ikut mengecil". Dua akar berbeda, dan yang kedua butuh **tiga ronde screenshot** sebelum ketemu bentuk yang dimaksud.

**Akar 1 — beban piksel.** Zoom-out di Retina = viewport CSS membesar + `devicePixelRatio` turun di bawah 1. Canvas ber-`dpr={[1, 1.5]}` menjepit dpr **ke atas** tapi lantai 1-nya justru meledakkan buffer saat dpr<1 (viewport 4356×2723 CSS × dpr 1 = 11,9 Mpx vs 1,3 Mpx normal). Fix: **`zoomDpr.ts`** — `zoomOutFactor = min(1, devicePixelRatio)` + hook `useDevicePixelRatio` (resubscribe `matchMedia('(resolution: Xdppx)')`, dpr ter-update tanpa reload) dipasang di EMPAT Canvas (Scene, IndustriesStack, ServicesTicker, InquiryLaptop). Di Scene faktor mengalikan HASIL termostat AdaptiveDpr di prop; override `?dpr=` TIDAK ikut dikalikan (angka A/B terpasang apa adanya). p95 zoom 33%: **66,6 → 17,3 ms**. Penjaga: `zoomDpr.test.ts` (unit murni + pemeriksaan teks 4 Canvas + override-tidak-dikalikan).

**Akar 2 — layout diukur satuan viewport.** Konten yang diukur `vw/svh/w-full` membesar BERSAMA viewport CSS saat zoom-out → terlihat "tidak ikut zoom". Fix bertingkat (kronologis, karena dua ronde pertama ternyata belum menjawab keluhannya):

1. Utility **`section-shell`** (`mx-auto w-full max-w-[1600px]`, satu angka di index.css) dipasang di 12 section top-level + clamp foto Vision (`w-[90vw] max-w-full sm:max-h-[900px]`).
...
- **`CareerRole`**: `meta` string tunggal ("Full-time · Remote · Growth") dipecah jadi `type` (departemen saja — lokasi sengaja tidak ditampilkan, jangan digabung lagi) + `status: "open" | "closed"`.
- **Baris closed** (`ClosedRoleRow`): teks abu-abu statis — bukan `<button>`, tanpa hover, tanpa preview foto, tanpa accordion; overview/skills tidak dirender sama sekali. Membuka lagi = ganti `status` ke `"open"`, kontennya sudah siap. Cabang open/closed dipilih di pemetaan daftar, BUKAN early-return dalam `RoleItem` — RoleItem memanggil hook (preview pengikut kursor) yang tidak boleh dilewati kondisional.
- **Header tabel Role/Type** + `ROW_GRID` bersama (header, baris open, baris closed memakai template grid yang PERSIS sama). 🐛 Kolom status wajib lebar **TETAP 5rem**, jangan `auto`: tiap baris adalah grid TERPISAH, track `auto` diukur per baris dari isinya sendiri (header kosong 0px, "(closed)" 54px, "(open) →" 61px) → kolom Type jatuh di tiga posisi x berbeda — persis keluhan "Type tidak sejajar".
- **Isi**: 3 role lama (Innovation & Growth Manager, Technical Lead, Product Builder) ditutup; Full Stack Engineer tetap open; role baru **Accountant** (Finance) & **Customer Success** — keduanya open. Test menyusul: baris closed inert & tanpa detail, header tabel tampil.

---

## 4bd. Form Inquiry Benar-benar Mengirim: Web3Forms + Peringatan Isian ✅ (26 Agu)

Commit `7687cf6`. **Blocker rilis terakhir (§4ac) tertutup.** `submitInquiry()` berhenti jadi stub 900 ms dan mulai mengirim sungguhan ke `api.web3forms.com/submit`.

### Yang dipasang

- **Access key sama dengan situs V1** (`Project/Apa-ini/index.html:2290`). **Publik by design** — Web3Forms memang menaruhnya di kode klien, dan catatan V1 sendiri sudah menyatakan itu bukan secret. Jadi dua situs berbagi satu key dan satu jatah **250 kiriman/bulan**; yang membedakan asal kiriman cuma `from_name`: V1 `"cogniti website"`, V2 **`"cogniti website (V2)"`**.
- **Tujuannya `hello@cogniti.id`**, bukan email pribadi Keano. ⚠️ Catatan V1 (`Apa-ini/Documentation.md:358-365`) menyebut `keanobramantyaalmaliki@gmail.com` — **itu sudah basi**, dasbor Web3Forms yang benar. Jangan pakai catatan V1 sebagai sumber kebenaran soal tujuan.
- **Badan kiriman dirakit eksplisit, BUKAN sebar `...payload`.** Tiap kunci di objek itu muncul sebagai satu baris di email, jadi bentuk emailnya ditentukan di satu tempat. 🔑 `interests` **wajib** diubah jadi teks dulu — array yang dilempar apa adanya tampil sebagai objek di badan email.
- **Tiga penjaga**, ketiganya diambil dari V1 dan alasannya bertahan:
  - **Honeypot `botcheck`** — kiriman dibuang **diam-diam TAPI dilaporkan `ok: true`**, supaya bot mengira berhasil dan tidak mencoba jalur lain. Disembunyikan dengan **menggeser ke luar layar**, bukan `display:none`: sebagian bot melewati field yang jelas-jelas disembunyikan. ⚠️ `autoComplete="off"` + `tabIndex={-1}` + `aria-hidden` wajib ketiganya — autofill browser di field ini akan membuang kiriman **tamu sungguhan** tanpa jejak, dan gejalanya identik dengan "email tidak sampai".
  - **Cooldown 5 menit** lewat `localStorage` (`cogniti_last_sent`). 🔑 Cap waktunya ditulis **HANYA setelah sukses** — kiriman yang gagal tidak boleh menghabiskan jatah orangnya. Pembacaan localStorage yang melempar (Safari private) dijawab "boleh kirim": anti-spam yang rusak tidak boleh berubah jadi pintu terkunci.
  - **Timeout 15 dtk** lewat `AbortController`. Tanpa ini tombolnya bisa berhenti selamanya di "Sending…" dan pengunjung tidak tahu harus apa.

### 🐛 Peringatan email tak sah yang MUSTAHIL tampil

Keano mengetik `test` di kolom email dan tidak mendapat apa-apa — tombolnya cuma mati diam-diam ("nanti bakal membuat user bingung kok nggabisa dikirim padahal udah keisi semua").

Penyebabnya **struktural, bukan salah ketik**, dan bentuknya melingkar:

| | |
|---|---|
| `ContactForm.tsx:193` | tombol kirim **dimatikan** selama `invalid` |
| `ContactForm.tsx:230` | pesan galat **hanya dirender** saat `status === "error"` |
| | `status` cuma bisa jadi `"error"` lewat **percobaan kirim** — percobaan yang justru dihalangi tombol mati di baris 193 |

Jadi pesannya tidak "kadang tidak muncul": ia **tidak akan pernah bisa muncul** selama isiannya tak sah, yaitu tepat saat ia dibutuhkan. Validasinya benar sejak awal, jalur tampilnya yang tidak pernah ada.

Obatnya galat **per-isian** (`inquiryFieldErrors`) yang muncul saat isian **DITINGGALKAN** (blur), bukan saat mengetik — memerahkan kolom yang belum sempat disentuh sama saja memarahi orang sebelum ia mulai. Ditambah garis bawah merah + `aria-invalid`. `validateInquiry()` lama dipertahankan tapi kini **diturunkan dari** `inquiryFieldErrors` lewat `FIELD_ORDER`: tombol dan penjaga kirim tidak boleh punya pendapat berbeda soal apa yang sah.

### 🔥 Tiga jebakan yang memakan waktu

- **Kelas border Tailwind tidak menang karena ditulis belakangan.** `` `${FIELD} border-red-400` `` di mana `FIELD` sudah memuat `border-white/25` → yang menang urutan **stylesheet**, bukan urutan di atribut. Penandanya bisa diam-diam tidak kelihatan. Dipisah jadi `FIELD_BASE` + `FIELD_OK`/`FIELD_BAD`, dipilih salah satu, tidak pernah dua-duanya.
- **React mendengarkan `focusout`, bukan `blur`.** Probe pertama melaporkan `aria-invalid: false` dan tidak ada peringatan — terbaca seperti fix yang gagal. Sebabnya probe-nya: `dispatchEvent(new FocusEvent('blur'))` **tidak** memicu `onBlur` React, karena React mendelegasikan lewat `focusout` yang menggelembung. Pakai `el.focus()` / `el.blur()` sungguhan. Kodenya tidak pernah salah.
- **Web3Forms MENOLAK panggilan dari server.** `curl` ke endpoint-nya dijawab **403** `"This method is not allowed. Use our API in client side"` (paket gratis). Artinya key-nya **tidak bisa diverifikasi dari terminal sama sekali** — satu-satunya cara menguji jalur kirim adalah menyetir browser sungguhan lewat CDP.

### Bahasa & verifikasi

Semua pesan yang dilihat pengunjung **berbahasa Inggris** mengikuti form-nya (komentar kode tetap Indonesia, sesuai konvensi repo) dan **tanpa em dash** (sejalan §4as). Tiga test menjaga kalimat Indonesia tidak menyelinap balik ke teks pengunjung lewat regex kata-kunci. `humanizeWait` menangani bentuk tunggal: "1 minute", bukan "1 minutes".

- **`src/lib/contact/submitInquiry.test.ts`** — 29 test: penjaga sebelum jaringan, honeypot dibuang-diam, cooldown (termasuk "gagal tak menghabiskan jatah" & localStorage yang melempar), bentuk kiriman, ragam kegagalan (`success:false`, jaringan putus, jawaban non-JSON, abort), dan peringatan per-isian — satu test **mengunci kasus `"test"` yang dilaporkan Keano**.
- **`scripts/probe-inquiry-send.mjs`** (baru) — bedah **bertingkat**: isi honeypot → fetch dipanggil/tidak → badan kiriman → jawaban server. Bertingkat karena form ini bisa "berhasil" di layar sambil tidak mengirim apa pun (honeypot ter-autofill), dan gejalanya identik dengan kiriman hilang. Aman by default, `--live` untuk kirim sungguhan.

> ⚠️ **`scripts/probe-contact-form.mjs` ikut diamankan di commit yang sama.** Dulu ia menekan tombol kirim yang cuma stub; begitu Web3Forms terpasang, **tiap kali probe itu jalan ia mengirim email sungguhan** dan memakan jatah 250/bulan — efek samping yang tidak kelihatan dari diff mana pun. Karena yang dibuktikan di sana adalah **rantai masukan** (pointer menembus canvas), bukan pengirimannya, fetch ke web3forms disadap dan dijawab tiruan. Jeda 800 ms di stub itu **disengaja**: jawaban seketika membuat keadaan "Sending…" lewat dalam satu frame dan check-nya jatuh karena stub-nya terlalu cepat, bukan karena UI-nya salah. Cooldown ikut dibersihkan tiap jalan, kalau tidak dua kali jalan beruntun gagal dengan alasan yang salah. `--live` tetap ada untuk uji ujung-ke-ujung.

**Terverifikasi**: 2 kiriman uji tercatat di dasbor Web3Forms (`2/250`), badan kiriman benar & honeypot kosong (probe), peringatan tampil di browser sungguhan. **Yang belum terverifikasi**: apakah emailnya benar-benar mendarat di kotak masuk `hello@cogniti.id` — kotak itu dipegang rekan kerja Keano, menunggu konfirmasi.
...
## 6d. Minigame Billiard ✅ **FINAL** (dibangun 28 Jul; direview & ditutup 12 Agu; dibuka lagi 23 Agu untuk free ball — §4ay)

Sandbox: aim + power + tembak, bola masuk lubang hilang, auto re-rack, tombol reset, **free ball di zona kitchen** (sejak 23 Agu, §4ay). **Tanpa skor/giliran/aturan 8-ball.** Fisika **cannon-es 0.20** — dipakai LANGSUNG tanpa wrapper R3F (lihat di bawah kenapa pindah dari Rapier).
...
- **Vitest 4** — `bun run test`. **319 test di 52 berkas**, semuanya hijau per 26 Agu (322/53 turun ke 308/51 saat PhysicsHeading + AwardsShowcase dicabut §4ba, naik lagi lewat `zoomDpr.test.ts` §4bb + test Careers closed §4bc). Empat di antaranya invariant lintas-wilayah (`INVARIANTS.md` §1, §3, §6, §7). Norma repo: **buktikan test-nya MERAH di kondisi rusak dulu** sebelum dipakai memverifikasi perbaikan
