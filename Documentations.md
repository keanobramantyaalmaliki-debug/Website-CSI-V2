...
> ⚠️ **Test suite: 318 test / 52 berkas, hijau** (23 Agu; turun dari 324/53 karena carousel `IndustriesMobile` pensiun membawa test-nya, §4aw — minus 10 test carousel, plus 4 test navigasi sektor). Satu test *pernah* merah pada satu putaran penuh — `TheCrewMobileCarousel.test.tsx` ("auto-advances 30s after going idle") — lalu hijau saat dijalankan sendiri **dan** pada putaran penuh berikutnya. **Flake fake-timer di bawah beban, bukan regresi**; kalau ia muncul lagi, curigai `advanceTimersByTimeAsync` di suite yang berbagi sesi timer, bukan komponennya.
...
- **Vitest 4** — `bun run test`. **318 test di 52 berkas**, semuanya hijau per 23 Agu (sempat 324/53 pada 21 Agu; turun karena test carousel `IndustriesMobile` ikut pensiun, §4aw). Empat di antaranya invariant lintas-wilayah (`INVARIANTS.md` §1, §3, §6, §7). Norma repo: **buktikan test-nya MERAH di kondisi rusak dulu** sebelum dipakai memverifikasi perbaikan
