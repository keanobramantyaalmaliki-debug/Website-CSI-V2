...
/**
 * Regression guard for the mobile horizontal-overflow fix: these six
 * sections host elements wide enough to overflow a 360px viewport (sticky
 * columns, the physics heading, the trusted-by hover-swap ghost). Each
 * section root must clip the x-axis locally so any residual overflow never
 * escapes to the document — `overflow-x-clip`, not `overflow-hidden`,
 * because Industries and LivingArchitecture contain `lg:sticky` children
 * that `overflow: hidden` would break.
 */
const SECTIONS: { name: string; id: string; Component: () => React.JSX.Element }[] = [
  ...
  { name: "TrustedBy", id: "trusted-by", Component: TrustedBy },
